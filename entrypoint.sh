#!/bin/ash
	...
	echo "Finding WireGuard configuration files..."
	configs=`find /etc/wireguard -type f -printf "%f\n"`
	if [[ -z "$configs" ]]; then
		echo "No configuration file found in /etc/wireguard" >&2
		exit 1
	fi
	echo "Found configuration files: $configs"

	config=`echo $configs | head -n 1`
	interface="${config%.*}"
	echo "Using configuration: $config, interface: $interface"

	echo "Checking sysctl net.ipv4.conf.all.src_valid_mark..."
	if [[ "$(cat /proc/sys/net/ipv4/conf/all/src_valid_mark)" != "1" ]]; then
		echo "sysctl net.ipv4.conf.all.src_valid_mark=1 is not set" >&2
		exit 1
	fi
	echo "sysctl net.ipv4.conf.all.src_valid_mark is set"

	echo "Modifying wg-quick script..."
	sed -i "s:sysctl -q net.ipv4.conf.all.src_valid_mark=1:echo Skipping setting net.ipv4.conf.all.src_valid_mark:" /usr/bin/wg-quick

	echo "Starting WireGuard..."
	wg-quick up $interface

	echo "Setting up IPv4 kill switch..."
	...
	echo "Setting up IPv6 kill switch..."
	...
	echo "Allowing traffic to local subnets..."
	...
	qbittorrent-nox --webui-port=8080 && sleep infinity &
	wait $!